...
Antes de desplegar una Saga, verifica si el proceso de negocio puede ser contenido dentro de un **Único Contexto Delimitado**. Si es así, IMPONER el uso del **Patrón Unit of Work** (ADR-0019) para ejecutar una transacción ACID estándar localmente. Esto se prefiere el 100% de las veces.

### 2. Paso de Evolución: La Condición de Aplicabilidad de la Saga
Mandar una implementación de Saga ÚNICAMENTE cuando:
1.  La transacción deba abarcar **dos o más bases de datos de microservicios separadas** (Particionadas físicamente según ADR-0031).
...
*   **Coreografía (Saga Dirigida por Eventos)**: Recomendación estándar para cadenas cortas (2 a 3 pasos). Los servicios escuchan el Bus de Eventos (ADR-0015) y reaccionan directamente a los eventos de finalización/fallo. Sin controlador central.
...
- **Transactional Outbox** (ADR-0033): Para garantizar que el evento inicial de arranque nunca se pierda.
...
- Depurar los flujos de trabajo entre servicios es más complejo, dependiendo en gran medida de la correlación unificada de trazas distribuidas (ADR-0007).
